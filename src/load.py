...
def load(kinesis_client, stream_name, records):
    ...
    Given a stream name and records, writes the records to the steam in JSON format
    ...
    if stream_status != "ACTIVE":
        return {"ok": False, "reason": "Stream not ACTIVE", "status": stream_status}
    ...
    kinesis_list = [{   'Data': json.dumps(record).encode("utf-8"),
                        'PartitionKey': 'pk'
