...
@pytest.fixture(autouse=True)
def secrets_manager():  # creates a Secrets client & cleans up after use
    with mock_aws():
        yield boto3.client(service_name="secretsmanager",
...
def mocked_api_and_secrets():
    with patch(
        "src.extract.get_secret", return_value="fake_key"
    ) as mock_get_secret, patch("src.extract.requests.get") as mock_get:
...
def test_extract_function_handles_empty_results(
    mocked_api_and_secrets, secrets_manager
):
    # Arrange
    input = "hello"
    mocked_api_and_secrets[0].return_value.json.return_value = {
        "response": {"results": []}
    }
    # Act
...
def test_extract_passed_valid_date_to_api_params(
    mocked_api_and_secrets, secrets_manager
):
    # Arrange
    input = "hello world"
...
def test_url_string_requires_international_format_date_string_with_hyphen(
    mocked_api_and_secrets, secrets_manager
):
    # Arrange
...
def test_extract_api_connection_returns_a_200_status_code(
    mocked_api_and_secrets, secrets_manager
):
    # Arrange
    input = "hello world"
...
def test_extract_api_connection_handles_500_status_code(
    mocked_api_and_secrets, secrets_manager
):
    # Arrange
    mocked_api_and_secrets[0].return_value.status_code = 500
    mocked_api_and_secrets[0].return_value.raise_for_status.side_effect = (
...
def test_extract_calls_get_secrets_function_and_retrieves_the_secret(
    mocked_api_and_secrets, secrets_manager
):
    input = "hello world"
    ...
    mocked_api_and_secrets[1].assert_called_once_with(
        secrets_manager, "guardian-api")
